...
import { StepType, FunnelStep, FormData, VerificationState } from "./types";
import {
  ChevronDown,
  ChevronRight,
  ChevronLeft,
...
const FUNNEL_STEPS: FunnelStep[] = [
  ...
  },
  {
    ...
    subtext:
      "We sent a 6-digit code to your phone number to verify your request.",
...
const App: React.FC = () => {
  ...
  const verifyAndSubmit = async () => {
    ...
    setIsSubmitting(true);
    const isApproved = await checkVerificationCode(
      formData.phone!,
      formData.otp,
    );
      ...
      <div className="min-h-screen flex flex-col animate-fade-in font-sans bg-[#f4fcfe]">
        ...
        <main className="flex-grow flex flex-col items-center justify-center py-20 px-4 text-center">
          ...
          <p className="text-lg md:text-xl text-[#034078] italic mb-16 opacity-90 max-w-2xl">
            One of our specialists from Can Rate Compare will contact you
            shortly to discuss your group benefits quote.
          </p>

          <div className="flex flex-col items-center space-y-6">
            <p className="text-sm font-bold text-[#034078]">
        ...
        <footer className="bg-[#031930] py-10 text-white text-center">
          <p className="text-sm font-bold">
            © 2025 Can Rate Compare. All Rights Reserved.
          </p>
        </footer>
      </div>
    );
  ...
  return (
    <div
      ref={topRef}
      className="max-w-2xl mx-auto px-4 py-12 flex flex-col items-center min-h-screen"
    >
      ...
      <div className="w-full bg-white rounded-[40px] shadow-2xl p-8 md:p-12 border border-slate-50 animate-slide-up relative">
        <div className="flex justify-between items-center mb-8">
          {currentStepIndex > 0 ? (
            <button
              onClick={prevStep}
              className="text-gray-400 hover:text-gray-600 font-bold flex items-center gap-1 text-sm uppercase tracking-wider transition-colors"
            >
          ...
          <span className="text-xs font-black text-gray-300 tracking-widest uppercase">
            Step {currentStepIndex + 1} of {FUNNEL_STEPS.length}
          </span>
        </div>

        <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-2 text-center leading-tight">
        ...
        <div className="w-full h-1.5 bg-gray-100 rounded-full mb-10 overflow-hidden">
          <div
            ...
            style={{
              width: `${((currentStepIndex + 1) / FUNNEL_STEPS.length) * 100}%`,
            }}
          />
        </div>
          ...
          <div className="grid gap-3">
            {currentStep.options?.map((opt) => (
              ...
              >
                ...
                {formData[currentStep.id] === opt && (
                  <CheckCircle2 className="w-6 h-6 shrink-0 absolute right-6" />
                )}
              </button>
            ))}
          </div>
        ...
        {currentStep.type === StepType.CONTACT_FORM && (
          <div className="space-y-4 max-w-md mx-auto">
            <input
              ...
              className="w-full p-5 border-2 border-gray-100 rounded-2xl outline-none focus:border-[#28a745] text-lg font-bold"
            />
            <input
              name="phone"
              type="tel"
            ...
            <button
              onClick={triggerSmsVerification}
              disabled={isSubmitting}
              className="w-full bg-[#dc3545] hover:bg-[#c82333] text-white py-5 rounded-2xl font-black text-xl shadow-xl transition-all flex justify-center items-center gap-2 uppercase tracking-tight transform hover:-translate-y-1"
            >
              {isSubmitting ? (
        ...
        {currentStep.type === StepType.OTP_VERIFICATION && (
          <div className="space-y-6 max-w-md mx-auto text-center">
            <input
              ...
              placeholder="000000"
              className="w-48 text-center p-5 border-4 border-gray-100 rounded-3xl text-5xl font-black font-mono focus:border-[#28a745] outline-none tracking-tighter"
            />
            {error && <p className="text-red-500 font-bold">{error}</p>}
            <button
              onClick={verifyAndSubmit}
              disabled={isSubmitting}
              className="w-full bg-[#28a745] text-white py-5 rounded-2xl font-black text-xl shadow-xl transition-all flex justify-center items-center gap-2 uppercase"
            >
              {isSubmitting ? (
            ...
            <button
              onClick={triggerSmsVerification}
              className="text-[#28a745] font-bold text-sm hover:underline"
            >
              Resend code
            </button>
          </div>
      ...
      <div className="mt-12 text-center pb-20 space-y-8">
        <p className="text-gray-800 font-black text-xl flex items-center justify-center gap-2">
          <Counter end={12.26} />+ Canadian Businesses Protected{" "}
          <Truck className="w-6 h-6 text-[#FBBC05] fill-[#FBBC05]" />
        </p>

        <div
          className="flex flex-col sm:flex-row justify-center items-center gap-6 sm:gap-12 animate-fade-in"
        ...
        <p className="text-gray-400 text-[10px] font-bold uppercase tracking-[0.3em]">
          Can Rate Compare &copy; {new Date().getFullYear()}. All Rights
          Reserved.
        </p>
      </div>
    </div>
